...
if (isset($_POST['submit'])) { // Form has been submitted.
	...
	if (DEMO_MODE == 'ON') {
		$message = "<div class='alert alert-warning'><button type='button' class='close' data-dismiss='alert'>×</button>对不起，您不能注册，演示模式已启用。</div>";
	} else {
		if ($username != "" && $password != "" && $repeat_password != ""  && $last_name != "" && $email != "" && $gender != "") {
		  if($password == $repeat_password){
			if(!$check_username){
				if(!$check_email){
					...
					$message = "<div class='alert alert-warning'><button type='button' class='close' data-dismiss='alert'>×</button>对不起，您的电子邮件地址已被注册。</div>";
				}
			} else {
				$message = "<div class='alert alert-warning'><button type='button' class='close' data-dismiss='alert'>×</button>对不起，您的用户名已被占用。</div>";
			}
		  } else {
			$message = "<div class='alert alert-warning'><button type='button' class='close' data-dismiss='alert'>×</button>密码不匹配。</div>";
		  }
	  	} else {
...
<?php $page_title = "注册"; require_once("includes/themes/".THEME_NAME."/header.php"); ?>
...
<div class="title">
	...
	<form action="register.php" method="POST" >
		...
		<div class="row-fluid">
			...
			<div class="span4">
				<label>姓名 <em class="req">*</em></label>
		        <input type="text" name="last_name" class="span12" required="required" value="<?php echo $last_name; ?>" />
			</div>
			<div class="span4">
				<label>用户名 <em class="req">*</em></label>
		        <input type="text" name="username" class="span12" required="required" value="<?php echo $username; ?>" />
			</div>
			<div class="span4">
				<label>电邮地址 <em class="req">*</em></label>
		        <input type="text" name="email" class="span12" required="required" value="<?php echo $email; ?>" />
			</div>
		</div>
		...
		<div class="row-fluid">
			...
			<div class="span4">
				<label>性别 <em class="req">*</em></label>
