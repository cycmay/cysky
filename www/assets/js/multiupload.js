		...
		if(this.items.length > 0){
			...
			var uId = "";
 			for(var i = 0; i<this.items.length; i++){
				...
				var sampleIcon = '<img src="assets/img/multi/image.png" />';
				var errorClass = "";
				if(typeof this.items[i] != undefined){
					if(self._validate(this.items[i].type) <= 0) {
						sampleIcon = '<img src="assets/img/multi/unknown.png" />';
						errorClass =" invalid";
					} 
					html += '<div class="dfiles'+errorClass+'" rel="'+uId+'"><h5>'+sampleIcon+this.items[i].name+'</h5><div id="'+uId+'" class="progress" style="display:none;"><img src="assets/img/multi/ajax-loader.gif" /></div></div>';
				}
			}
				...
				success:function(rponse){
					...
					$.each(obj,function(k,fle){
						if($(fle).attr("rel") == rponse){
							$("#uploaded").append("Successfully Uploaded: <strong>"+file[f].name+"</strong><br />");
