...
if(isset($_GET['search'])){
	...
	if($_GET['filter'] == "title"){
		$sql = "SELECT id,name,investment_wanted,amount_invested,description,thumbnail,expires FROM investments AND name LIKE '%$query%' $end ORDER BY created DESC";
	} else if($_GET['filter'] == "description") {
		$sql = "SELECT id,name,investment_wanted,amount_invested,description,thumbnail,expires FROM investments AND description LIKE '%$query%' $end ORDER BY created DESC";
	} else if($_GET['filter'] == "started_by") {
		$search_username = Investments::find_id_by_username($query);
		$sql = "SELECT id,name,investment_wanted,amount_invested,description,thumbnail,expires FROM investments AND creator_id LIKE '%$search_username%' $end ORDER BY created DESC";
	} else {
		if($end != ""){
			$end = "WHERE ".$end;
		}
		
		$sql = "SELECT id,name,investment_wanted,amount_invested,description,thumbnail,expires FROM investments $end  ORDER BY created DESC";
	}
	...
	$search = false;
	$query_data = Investments::find_by_sql("SELECT id,name,investment_wanted,amount_invested,description,thumbnail,expires FROM investments ORDER BY created DESC");
	$page = !empty($_GET['page']) ? (int)$_GET['page'] : 1;
	$per_page = 20;
	$total_count = count($query_data);
	$pagination = new Pagination($page, $per_page, $total_count);
	$sql = "SELECT id,name,investment_wanted,amount_invested,description,thumbnail,expires FROM investments ORDER BY created DESC LIMIT {$per_page} OFFSET {$pagination->offset()}";
	$query_data = Investments::find_by_sql($sql);	
}
...
<?php $page_title = "项目管理"; require_once("../includes/themes/".THEME_NAME."/admin_header.php"); ?>
	...
	<form action="projects.php" method="GET" class="form-search">
		<input type="text" placeholder="搜索..." name="search" class="input-xlarge" style="width:250px;">
	...
	<div class="row-fluid">
		...
		<div class="span3">
			<div class="well" style="max-width: 340px; padding: 8px 0;">
				<ul class="nav nav-list">
					...
					<?php foreach(Investments::get_categories() as $category){ ?>
					<li <?php if(isset($_GET['category']) && $_GET['category'] == $category->id){echo "class=\"active\" ";} ?>><a href="<?php echo WWW.ADMINDIR."projects.php?search=&filter=&category=".$category->id; ?>"><?php echo $category->name; ?></a></li>
					<?php } ?>
				</ul>
			</div>
		</div>
		...
		<div class="span9">
			...
			<?php if(empty($query_data)){ ?>
				<strong>对不起，我们无法为你的搜索找到任何结果。</strong>
			<?php } else { ?>
			<ul class="thumbnails">
				...
				<li class="span3">
					<div class="thumbnail" style="position: relative; height: 320px;">
						...
						<?php } ?>
						<img src="../assets/project/<?php echo $data->id."/images/".$data->thumbnail; ?>" style="height:130px" alt="Image" />
						<a href="<?php echo WWW.ADMINDIR."project.php?id=".$data->id; ?>">
				...
				echo "<div class='pagination pagination-centered'><ul>";
					...
					for($i=1; $i <= $pagination->total_pages(); $i++) {
						if($i == $page) {
							...
							echo " <li><a href=\"projects.php?search={$query}&amp;filter={$_GET['filter']}&amp;page={$i}\">{$i}</a></li> "; 
						}
					}
